...
export async function getAllData(dataCollection) {
	let results = await wixData.query('Ministries').include('serviceOpportunities').find();
...
export async function getMultiReferencePropertyFromCollection(multiRefColumn, collection, collectionId) {
    ...
    let response = await wixData.queryReferenced(collection,collectionId,multiRefColumn);
