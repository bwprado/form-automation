...
$w.onReady(() => {
    ...
    $w("#repeater1").onItemReady(($w, itemData, index) => {
        ...
        */
        ...
        itemData.actionButtonUrl ? $w("#buttonAction").expand() : $w("#buttonAction").collapse();

        // action button label defaults to Register, but can be overwriiten in the Collection
        itemData.actionButtonLabel ? $w("#buttonAction").label = buttonLabel : $w("#buttonAction").label = "Register"
...
function populateLocation() {
    ...
    wixData.query("LearningCommunity").ne("isHidden", true)
        .limit(1000)
        .ascending("day")
        .distinct("day")
        .then(results => {
...
function search() {
    ...
    $w('#datasetLearning').setFilter(filter)
        .then(count);
