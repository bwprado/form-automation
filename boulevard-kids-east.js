...
$w.onReady(async () => {
    ...
    await $w('#datasetEvents').setFilter(wixData.filter()
        .ge('eventStartDate', today)
        //.eq('ministrySpecificEvent', true)
        .hasSome('eventMinistries', ["b594a941-1bd9-45b6-859d-9fb59eba3433", "04b632be-e422-4648-afca-be5c2a468b0f"])
...
function onReadyActions() {
    ...
    $w("#repeaterStaff").onItemReady(($item, itemData, index) => {
        $item("#buttonContact").link = "mailto:" + itemData.staffEmail + "?subject=Boulevard Kids";
...
async function filterByResourceType(event) {
    ...
    } else if (resourceTypeSelection === RESOURCE_TYPE_SELECTION_SINGLE && indexOfResource === -1) {
    ...
    if (indexOfResource === -1) {
        ...
        await $w('#datasetBKResources').setFilter(wixData.filter()
        ...
        if (totalAfterFilter !== 0) {
            ...
            setResourcesLoadedMessage(`${$w('#datasetBKResources').getTotalCount()} resources`);
...
async function loadMoreResourcesToggle(event) {
    ...
    if (repeaterLength === 12) {
        await $w('#datasetBKResources').setPageSize($w('#datasetBKResources').getTotalCount());
    ...
    setResourcesLoadedMessage(`Resources available: ${$w('#datasetBKResources').getTotalCount()}, showing ${$w('#datasetBKResources').getPageSize()}`);
...
function loadItemInfo($item, itemData, index) {
    ...
    $item("#imageResource").src = image || "https://static.wixstatic.com/media/77e3c1_4bd0502e295b4b37b0489a60a878efc2~mv2.png";
