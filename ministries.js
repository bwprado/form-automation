...
async function cacheServiceOpportunities() {
    ...
    items.forEach(item => { serviceOpportunitiesById[item._id] = item.serviceOpportunities.length });
...
$w.onReady(async function () {
    ...
    $w("#datasetMinistries").onReady(() => {
        ...
        $w("#repeater1").onItemReady(async ($item, itemData, index) => {
            ...
            if (opportunities > 0) {
                $item("#buttonOpportunities").label = `${opportunities > 1? opportunities+" opportunities":opportunities+" opportunity"}`;
            ...
            if (image) {
                ...
                $item("#imageMinistry").src = "https://static.wixstatic.com/media/77e3c1_a3767b60e1624ea2a70c82f3c816a9f0~mv2.png";
            }
    ...
    $w("#datasetMinistries").setFilter(wixData.filter().ne("hideMinistry", true))
        .then(count2)
    /*
...
async function buildCampus() {
    ...
    let resCampus = await wixData.query('Ministries').ne("hideMinistry", true).limit(999).distinct("ministryType");
    // console.log({ resCampus });

    let res = await wixData.query("MinistryTypes").hasSome("_id", resCampus.items).limit(999).ascending("title").find();
    // console.log({ res })
    let options = [{
            label: "All",
            value: "all"
...
function filterTypeDropdown() {
    ...
    $w("#dropdownType").onChange(() => {
        ...
        if ($w("#dropdownType").value === "all") {
            ...
            $w("#datasetMinistries").setFilter(wixData.filter().hasSome("ministryType", $w("#dropdownType").value).ne("hideMinistry", true))
                .then(count)
